...
function startPiPolling(piId, plan) {
    ...
    (function poll() {
        ...
        if (retries >= maxRetries) {
            document.getElementById("draw-desc").textContent = "支付验证超时，请刷新页面重试。如已支付，请点击“我已支付”按钮";
            if (btn) {
                btn.disabled = false;
                btn.querySelector(".btn-text").textContent = "我已支付";
                btn.onclick = function() { startPiPolling(piId, plan); };
            }
            return;
        }
        retries++;
...
(function() {
    ...
    if (pi && plan) { showStep("step-draw"); document.getElementById("draw-desc").textContent = "正在验证支付..."; startPiPolling(pi, plan); window.history.replaceState({}, "", "/"); }
