...
function verifyPayment(sid, plan) {
    ...
    fetch("/api/verify-payment",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({session_id:sid,plan:plan})}).then(function(r){return r.json()}).then(function(d){if(d.success){sessionStorage.setItem("stripe_ok","1");localStorage.setItem("tarot_purchase",d.purchase_id);state.purchaseId=d.purchase_id;state.remaining=d.remaining;updateRemainingBadge();closePricingModal();var serverCat=d.category||"";var serverQ=d.question||"";if(serverCat){state.selectedCategory=serverCat;state.question=serverQ;fetch("/api/draw",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({count:3})}).then(function(r){return r.json()}).then(function(d2){state.drawnCards=d2.cards;d2.cards.forEach(function(card,i){var slot=document.getElementById("slot-"+i);if(slot){var front=slot.querySelector(".card-front");if(front){var img=front.querySelector("img");if(img){img.src=getCardImageUrl(card);if(card.orientation==="逆位")img.classList.add("reversed");}}}});showStep("step-draw");document.getElementById("draw-desc").textContent="占卜主题："+CATEGORY_NAMES[state.selectedCategory]+" —— 你的牌已抽好";document.querySelectorAll(".card-slot").forEach(function(s){s.classList.remove("flipped","draw-animate");});setTimeout(function(){var sl=document.querySelectorAll(".card-slot");sl.forEach(function(s,i){setTimeout(function(){s.classList.add("draw-animate");},i*200);});},100);setTimeout(function(){revealCards();_startReading();state.isProcessing=false;},1000);});}else{var cat=localStorage.getItem("tarot_cat");var q=localStorage.getItem("tarot_q");if(cat){state.selectedCategory=cat;state.question=q||"";localStorage.removeItem("tarot_cat");localStorage.removeItem("tarot_q");_doDrawAndRead(false);}else{showStep("step-category");state.isProcessing=false;}}}}).catch(function(){state.isProcessing=false;});
...
(function() {
    ...
    // If no URL params, check localStorage for pending payment (manual redirect)
    if (!pi && !sid) {
        pi = localStorage.getItem("tarot_intent_id");
        plan = localStorage.getItem("tarot_intent_plan");
        if (pi) {
            localStorage.removeItem("tarot_intent_id");
            localStorage.removeItem("tarot_intent_plan");
        }
    }
